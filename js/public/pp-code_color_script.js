...
function ppCodeColor() {
  ...
  for (j = 0; j < k; j++) {
    ...
    l = x.length;
    for (i = 0; i < l; i++) {
      x[i].innerHTML = ppCodeColorize(x[i].innerHTML, modes[j]);
    }
  }
}
function ppCodeColorize(x, lang) {
  ...
  function htmlMode(txt) {
    ...
    php = new extract(rest, "&lt;\\?php", "?&gt;", phpMode, "PPPHPPOS");
    rest = php.rest;
    comment = new extract(rest, "&lt;!--", "--&gt;", commentMode, "PPHTMLCOMMENTPOS");
    rest = comment.rest;
    ...
    for (i = 0; i < comment.arr.length; i++) {
        rest = rest.replace("PPHTMLCOMMENTPOS", comment.arr[i]);
    }
    for (i = 0; i < php.arr.length; i++) {
        rest = rest.replace("PPPHPPOS", php.arr[i]);
    }
    return rest;
  }
  ...
  function cssMode(txt) {
    ...
    comment = new extract(rest, /\/\*/, "*/", commentMode, "PPCSSCOMMENTPOS");
    rest = comment.rest;
    while (rest.search("{") > -1) {
      s = rest.search("{");
    ...
    rest = rest.replace(/}/g, "<span style=color:" + cssdelimitercolor + ">}</span>");
    for (i = 0; i < comment.arr.length; i++) {
        rest = rest.replace("PPCSSCOMMENTPOS", comment.arr[i]);
    }
  ...
  function jsMode(txt) {
    ...
    for (i = 0; i < esc.length; i++) {
      rest = rest.replace("PPJSESCAPE", esc[i]);
    }
