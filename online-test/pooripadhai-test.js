...
function ReSizer(){$('textarea.resizable:not(.processed)').TextAreaResizer();$('iframe.resizable:not(.processed)').TextAreaResizer();}
...
//window.onerror=silentErrorHandler;   
 ...
 {
     try { $(".facebox").facebox(); } catch(Err){}
     
     ReSizer();
     
     PageHeadingBeautifier();
     
     BixPageSpecificFunctionsLoad();
 }

/* H1 Tag Beautifier */ 
function PageHeadingBeautifier()
{
   try
   {
       var tmpHeading = $("h1").html();
       
       if(tmpHeading.indexOf('-') > 1)
       {
...
{ 
    ...
    {      
        ...
        if( optNo == ansNo ) // Show Answer & Description with Tick mark image..
        {
            ...
            if(txtAnswerHtml.indexOf('http://www.indiabix.com/403.html') == -1)
            {
...
function OnlineCompiler(pQuesID)
{
  ...
  $('div#divCompilerOutput_' + pQuesID).html('<img  alt="Compilation in progress.." />');  
